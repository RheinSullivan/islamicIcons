			...
			<div class="gsap-container">
				...
				>
					{#each translation.home.heroWords as word, wordIndex (wordIndex)}
						<span class="hero-word inline-block {wordIndex === translation.home.heroWords.length - 1 ? 'text-islamic-green' : ''}">{word}{#if wordIndex < translation.home.heroWords.length - 1}&nbsp;{/if}</span>
					{/each}
				</h1>
			...
			{#each featuredIcons as item (item.name)}
				...
				<button type="button" data-open={item.name} class="gsap-on-scroll group w-full cursor-pointer text-left">
					<span class="relative block aspect-square overflow-hidden rounded-2xl border border-islamic-line bg-islamic-panel transition duration-300 group-hover:-translate-y-1 group-hover:border-islamic-line-strong group-hover:bg-islamic-panel-2 group-hover:shadow-[0_22px_55px_rgba(0,0,0,.22)]">
						<span class="absolute left-3 top-3 z-10 rounded-full border border-islamic-line bg-islamic-bg/80 px-2 py-1 text-[8px] uppercase tracking-[.12em] text-islamic-dim backdrop-blur">{(source?.label || sourceLabel(source?.id || '')).replace(/^Community\s*\/\s*/i, '')}</span>
