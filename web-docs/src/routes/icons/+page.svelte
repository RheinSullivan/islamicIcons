	...
	<div class="grid gap-10 lg:grid-cols-[220px_minmax(0,1fr)]">
		<aside class="lg:sticky lg:top-28 lg:h-fit">
				...
				{#each categories as cat (cat)}
					...
					>
						...
						<span class="text-[10px] text-islamic-dim"
							>{icons.filter((iconCandidate) => iconCandidate.category === cat).length}</span
						>
					</button>
				{/each}
			</div>
			...
			<div class="grid gap-1">
				...
				>
					...
					<span class="text-[10px] text-islamic-dim"
						>{icons.filter((iconCandidate) => iconCandidate.sources.some((sourceCandidate) => sourceCandidate.id === 'community')).length}</span
					>
				</button>
				<button
					type="button"
				...
				>
					...
					<span class="text-[10px] text-islamic-dim"
						>{icons.filter((iconCandidate) => iconCandidate.sources.some((sourceCandidate) => sourceCandidate.id === 'svg-repo')).length}</span
					>
				</button>
			</div>
		</aside>
		...
		<section class="min-w-0">
			...
			<div class="mt-8 flex flex-col gap-3 sm:flex-row">
				...
				>
					{#each ['fill', 'outline', 'color'] as variantChoice (variantChoice)}
						<button
							type="button"
							data-var={variantChoice}
							onclick={() => setVariant(variantChoice as 'fill' | 'outline' | 'color')}
							aria-pressed={variant === variantChoice}
							class="rounded-lg px-3 py-2.5 text-[10px] transition {variant === variantChoice
								? 'bg-islamic-green/10 text-islamic-green'
								: 'text-islamic-muted hover:text-islamic-text'}">{variantName(variantChoice)}</button
						>
					{/each}
				</div>
			</div>
