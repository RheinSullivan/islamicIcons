<script lang="ts">
	...
	const item = $derived(icons.find((catalogItem) => catalogItem.name === iconName) ?? null);
	...
	const source = $derived.by(() => {
		if (!item) return null;
		if (activeSource) return item.sources.find((sourceCandidate) => sourceCandidate.id === activeSource) ?? item.sources[0];
	...
	const availableVariants = $derived.by(() => {
		if (!source) return [];
		return (source.variants ?? []).filter((variantOption) =>
	...
	const assetPath = $derived(
		item && source ? `atsarul-mujahidin/icons/${item.category}/${item.name}/${variant}.svg` : ''
	);
...
<svelte:head>
	{#if item}
		<title>{item.title} Icon | Atsarul Mujahidin</title>
		<meta
			name="description"
...
<main class="{MAX} pt-32 pb-14 sm:pt-36 sm:pb-20">
			...
			<div>
				...
				{#if availableVariants.length > 1}
					<div class="mt-6">
						...
						<div class="flex flex-wrap gap-2">
							{#each availableVariants as variantOption (variantOption)}
								<button
									type="button"
									onclick={() => (activeVariant = variantOption)}
									aria-pressed={variantOption === variant}
									class="rounded-xl border px-4 py-2 text-[11px] transition {variantOption === variant
									...
									>{variantName(variantOption)}</button
								>
							{/each}
						</div>
					</div>
				...
				{#if item.sources.length > 1}
					<div class="mt-6">
						...
						>
						<div class="flex flex-wrap gap-2">
							{#each item.sources as sourceCandidate (sourceCandidate.id)}
								<button
									type="button"
									onclick={() => (activeSource = sourceCandidate.id)}
									aria-pressed={sourceCandidate.id === source?.id}
									class="rounded-xl border px-4 py-2 text-[11px] transition {sourceCandidate.id === source?.id
										? 'border-islamic-green/30 bg-islamic-green/8 text-islamic-green'
										: 'border-islamic-line text-islamic-muted hover:text-islamic-text'}"
									>{sourceCandidate.label}</button
								>
							{/each}
						</div>
					</div>
			...
			<div>
				...
				{#if item.aliases && item.aliases.length > 0}
					<div class="mt-4 flex flex-wrap gap-2">
						{#each item.aliases as aliasWord (aliasWord)}
							<span
				...
				<div class="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3">
					...
					<div class="rounded-xl border border-islamic-line p-4">
						...
						<b class="mt-2 block text-sm">{availableVariants.length}</b>
					</div>
				</div>
		...
		{#each [icons.filter((catalogItem) => catalogItem.category === item.category && catalogItem.name !== item.name).slice(0, 8)] as relatedIcons (0)}
			...
			<section class="mt-16 border-t border-islamic-line pt-12">
				...
				<div class="mt-8 grid grid-cols-2 gap-x-4 gap-y-9 sm:grid-cols-4 lg:grid-cols-6">
					{#each relatedIcons as relatedIcon (relatedIcon.name)}
						<a
							href="/icons/{relatedIcon.name}"
							class="gsap-on-scroll group text-center"
							aria-label="{relatedIcon.title} icon"
						>
							<span
							...
							>
								<DynamicIcon item={relatedIcon} variant="fill" class="size-full p-8 transition duration-500 group-hover:scale-105" />
							</span>
							<span class="mt-2 block text-[11px] font-medium text-islamic-muted group-hover:text-islamic-text"
								>{relatedIcon.title}</span
							>
						</a>
					{/each}
				</div>
