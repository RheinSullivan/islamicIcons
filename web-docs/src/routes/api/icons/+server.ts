...
export const GET: RequestHandler = async ({ url }) => {
	...
	if (source && source !== 'all') {
		filteredIcons = filteredIcons.filter(icon =>
			icon.sources.some(sourceCandidate => sourceCandidate.id === source)
		);
	}
	...
	if (variant) {
		filteredIcons = filteredIcons.filter(icon =>
			icon.sources.some(sourceItem =>
				sourceItem.variants && sourceItem.variants.includes(variant)
			)
		);
	}
	...
	const apiIcons = paginatedIcons.map(icon => ({
		...
		sources: icon.sources.map(sourceItem => ({
			id: sourceItem.id,
			label: sourceItem.label,
			variants: sourceItem.variants || [],
			solidVariants: sourceItem.solidVariants || []
		}))
	}));
