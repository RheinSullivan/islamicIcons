<script lang="ts">
	...
	const RESOURCES = $derived([
		...
		{ href: `/${locale}/brand-logos`,      label: locale === 'en' ? 'Brand Logos' : locale === 'id' ? 'Logo Brand' : locale === 'ar' ? 'شعارات العلامات' : 'ブランドロゴ' },
	...
	const drawerItem = $derived.by(() => {
		...
		if (!item) return null;
		const sourceIdentifier = drawerSource && item.sources.some(sourceCandidate => sourceCandidate.id === drawerSource)
			? drawerSource
			: sourceForItem(item)?.id;
		const source = item.sources.find(sourceCandidate => sourceCandidate.id === sourceIdentifier) || item.sources[0];
		const availableVariants = [...new Set([...(source?.variants || [])])].filter(variantCandidate =>
			['fill','outline','color','original','alternate'].includes(variantCandidate)
		);
...
<atsarul-mujahidin-icon name="${item.name}" variant="${variant}" size="32"><\/atsarul-mujahidin-icon>`
		...
		return { item, source, variant, vars: availableVariants, contributorName, frameworkExamples };
	});
	...
	// Check if current page is under Resources submenu
	const isResourcesActive = $derived(
		RESOURCES.some(resource => pathname.startsWith(resource.href))
	);

	const ALL_LOCALES: { code: Locale; label: string; flag: string }[] = [
	...
	function onGlobalClick(clickEvent: MouseEvent) {
		const clickTarget = clickEvent.target as HTMLElement;
		const openTriggerElement = clickTarget.closest('[data-open]') as HTMLElement | null;
		if (openTriggerElement) { drawerName = openTriggerElement.dataset.open!; drawerSource = null; return; }
		const closeTriggerElement = clickTarget.closest('[data-close]') as HTMLElement | null;
		if (closeTriggerElement) { drawerName = null; drawerSource = null; return; }
		const copyTriggerElement = clickTarget.closest('[data-copy]') as HTMLElement | null;
		if (copyTriggerElement) { copyText(copyTriggerElement.dataset.copy!); return; }
		const searchTriggerElement = clickTarget.closest('[data-search]') as HTMLElement | null;
		if (searchTriggerElement) { searchOpen = true; return; }
		const mobileToggleElement = clickTarget.closest('[data-mobile-toggle]') as HTMLElement | null;
		if (mobileToggleElement) { mobileOpen = !mobileOpen; return; }
		const drawerSourceElement = clickTarget.closest('[data-drawer-source]') as HTMLElement | null;
		if (drawerSourceElement) { drawerSource = drawerSourceElement.dataset.drawerSource!; return; }
		const drawerVariantElement = clickTarget.closest('[data-drawer-var]') as HTMLElement | null;
		if (drawerVariantElement) { currentVariant = drawerVariantElement.dataset.drawerVar!; return; }
	}
	...
	onMount(() => {
		...
		function onKeydown(keyboardEvent: KeyboardEvent) {
			if ((keyboardEvent.ctrlKey || keyboardEvent.metaKey) && keyboardEvent.key.toLowerCase() === 'k') { keyboardEvent.preventDefault(); searchOpen = true; }
			if (keyboardEvent.key === 'Escape') {
				if (drawerName) { drawerName = null; }
	...
	$effect(() => {
		...
		void pathname; void drawerName; void drawerSource; void currentVariant; void mobileOpen; void searchOpen;
	...
	function animatePage() {
		...
		const gsapContext = gsap.context(() => {
			...
			document.querySelectorAll('.gsap-on-scroll').forEach((scrollElement) => {
				if ((scrollElement as HTMLElement).dataset.gsapReady) return;
				(scrollElement as HTMLElement).dataset.gsapReady = '1';
				...
				windowGlobal.__islamicIconsObservers.push(scrollObserver);
				scrollObserver.observe(scrollElement);
			});
		});
		windowGlobal.__islamicIconsGsapContext = gsapContext;
	}
</script>
...
<svelte:head>
	...
	<link rel="alternate" hreflang="ar" href={pathname.replace(`/${locale}`, '/ar')} />
	<link rel="alternate" hreflang="ja" href={pathname.replace(`/${locale}`, '/ja')} />
...
<div class="min-h-screen overflow-x-clip bg-islamic-bg text-islamic-text">
		...
		<div class="{MAX} relative flex h-20 items-center justify-between gap-4">
			...
			<nav class="absolute left-1/2 hidden -translate-x-1/2 items-center gap-1 md:flex" aria-label="Primary">
				{#each NAV as item (item.key)}
					<a
						...
						class="flex h-20 items-center border-b-2 px-2 text-[13px] font-medium transition {isActive(item.key) ? 'border-islamic-green text-islamic-green' : 'border-transparent text-islamic-muted hover:border-islamic-green hover:text-islamic-green'}"
					>{item.label}</a>
				{/each}
				<!-- Resources dropdown -->
				<div class="group relative flex h-20 items-center">
					<button type="button" class="flex h-20 cursor-pointer items-center gap-1 border-b-2 px-3 text-[13px] font-medium transition {isResourcesActive ? 'border-islamic-green text-islamic-green' : 'border-transparent text-islamic-muted hover:border-islamic-green hover:text-islamic-green'}">
						Resources
			...
			<div class="ml-auto flex items-center gap-2">
				...
				>
					...
					<span>{translation.search.placeholder}</span>
	...
	{#if drawerItem}
		<div class="fixed inset-0 z-[90]" data-drawer-root>
			...
			<aside
				class="absolute right-0 top-0 h-full w-full max-w-xl border-l border-islamic-line-strong shadow-[-30px_0_90px_rgba(0,0,0,.45)] flex flex-col overflow-hidden bg-[#09150f]"
			...
			>
				...
				<div class="flex shrink-0 items-center justify-between border-b border-islamic-line bg-[#09150f]/95 px-5 py-4 backdrop-blur-xl">
					...
					<div class="ml-4 flex shrink-0 gap-2">
						<a
							href="/{locale}/icons/{drawerItem.item.name}"
							aria-label="Open full icon detail page"
							title="Open full page"
							class="grid size-9 place-items-center rounded-lg border border-islamic-line text-islamic-muted transition hover:border-islamic-green hover:text-islamic-green"
						>
				...
				<div class="flex-1 overflow-y-auto">
					<div class="p-5 sm:p-6">
						<!-- Preview -->
						<div class="relative flex items-center justify-center overflow-hidden rounded-xl border border-islamic-line bg-[radial-gradient(circle_at_50%_50%,rgba(115,224,174,.03),transparent_70%)] p-10">
							<div class="w-full max-w-[50%] flex items-center justify-center text-islamic-text">
					...
					{#if drawerItem.vars.length > 1}
						<div class="mt-5">
							...
							<div class="flex flex-wrap gap-2">
								{#each drawerItem.vars as variantOptionKey (variantOptionKey)}
									<button 
										type="button" 
										data-drawer-var={variantOptionKey} 
										class="cursor-pointer rounded-lg border px-3 py-1.5 text-[11px] font-medium transition {variantOptionKey === drawerItem.variant ? 'border-islamic-green/30 bg-islamic-green/10 text-islamic-green' : 'border-islamic-line text-islamic-muted hover:border-islamic-line-strong hover:text-islamic-text'}"
									>
