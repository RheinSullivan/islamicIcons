<script lang="ts">
	import { icons, pathFor, pretty, esc } from '$lib/site';
	...
	const results = $derived.by(() => {
		...
		if (!q) return [];
		return icons
			.filter((i) => `${i.title} ${i.name} ${i.category}`.toLowerCase().includes(q))
			.slice(0, 10);
	});

	function onBackdropClick(e: MouseEvent) {
		const target = e.target as HTMLElement;
		if (target.closest('[data-search-close]') || target === e.currentTarget) {
			onclose();
		}
	}
	...
	function onResultClick(e: MouseEvent) {
		const target = e.target as HTMLElement;
		const btn = target.closest('[data-result]') as HTMLElement | null;
		if (btn) {
			onopenIcon(btn.dataset.result!);
			onclose();
		}
	}
</script>
...
>
	<div class="relative w-full max-w-2xl overflow-hidden rounded-2xl border border-islamic-line-strong bg-[#09150f] shadow-[0_30px_100px_rgba(0,0,0,.55)]">
		...
		<div class="max-h-[55vh] overflow-y-auto p-2" onclick={onResultClick}>
			{#if results.length === 0}
			...
			{:else}
				{#each results as item (item.name)}
					...
					>
						...
						>
							<img class="size-7 object-contain" src={pathFor(item, 'fill')} alt={item.title} />
						</span>
