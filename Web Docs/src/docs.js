(() => {
  ...
  const sourceLabel = id => ({ community: 'Community', 'svg-repo': 'SVG Repo', 'mary-akveo': 'Mary Akveo', 'google-material': 'Google Material Icons', 'community-upload': 'Community Upload' }[id] || pretty(id));
  const sourceFor = (item) => {
    if (state.source !== 'all') return item.sources.find(s => s.id === state.source) || item.sources[0];
    return item.sources.find(s => s.id === 'community') || item.sources[0];
  };
